...
RSpec.describe Ruboty::AiAgent::UserMcpClient do
  ...
  describe '#list_tools' do
    ...
    before do
      allow(Ruboty::AiAgent::HttpMcpClient).to have_received(:new).and_return(mock_http_client)
    end

    context 'when cache hit' do
      before do
        allow(mock_http_client).to have_received(:list_tools)
    ...
    context 'when cache miss' do
      before do
        allow(mock_http_client).to have_received(:list_tools).and_return(tools_data)
      end
  ...
  describe '#call_tool' do
    ...
    before do
      allow(Ruboty::AiAgent::HttpMcpClient).to have_received(:new).and_return(mock_http_client)
      allow(mock_http_client).to have_received(:call_tool)
  ...
  describe 'delegation methods' do
    ...
    %i[list_prompts list_resources ping initialize_session cleanup_session].each do |method|
      it "delegates #{method} to mcp_client" do
        allow(mock_http_client).to have_received(method)
    ...
    it 'delegates get_prompt to mcp_client' do
      allow(mock_http_client).to have_received(:get_prompt)
    ...
    it 'delegates read_resource to mcp_client' do
      allow(mock_http_client).to have_received(:read_resource)
